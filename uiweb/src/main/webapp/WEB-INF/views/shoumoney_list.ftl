		...
		<script type="text/javascript">
			$(function(){
				...
				$("#exportBtn").click(function(){
					$("#currentPage").val(1);
					//将提交地址action改为 /export_return_list.do
					$("#searchForm").prop("action","/export_return_list.do");				
					$("#searchForm").submit();
				});
				...
				<div class="col-sm-9">
					<form action="/borrowBidReturn_list.do" name="searchForm" id="searchForm" class="form-inline">
						...
						<div class="form-group">
							<label>时间范围</label>
							<input type="text" class="form-control beginDate" name="beginDate" value="${(q.beginDate?string('yyyy-MM-dd'))!''}"/>
						</div>
						<div class="form-group">
							<label></label>
							<input type="text" class="form-control endDate" name="endDate" value="${(q.endDate?string('yyyy-MM-dd'))!''}"/>
						</div>
						<div class="form-group">
						    <label>状态</label>
						    ...
						    <script type="text/javascript">
						    	$("[name=state] option[value='${(q.state)!''}']").attr("selected","selected");
						    </script>
						</div>
						<div class="form-group">
					...
					<div class="panel panel-default">
						<div class="panel-heading">
							...
							<a id="exportBtn" class="pull-right btn btn-primary btn-sm" href="javascript:void(0)">
								导出数据
							</a>
						...
						<table class="table">
							...
							<tbody>
								<#list pageResult.result as data>
									<tr>
										<td><a href="/callbackmoney_list?id=${data.bidRequestId}">${data.bidRequestTitle}</a></td>
